...
app.post('/api/video', async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ error: 'Video failed.', detail: error.message }); }
});
app.get('/api/video/:id', async (req, res) => {
  try {
    const r = await fetch('https://api.dev.runwayml.com/v1/tasks/' + req.params.id, { headers: { Authorization: 'Bearer ' + process.env.RUNWAY_API_KEY, 'X-Runway-Version': '2024-11-06' } });
    const data = await r.json();
    const status = data.status === 'SUCCEEDED' ? 'completed' : data.status === 'FAILED' ? 'failed' : 'in_progress';
    res.json({ ok: true, video: { status } });
  } catch (error) { res.status(500).json({ error: 'Status failed.' }); }
});
app.get('/api/video/:id/url', async (req, res) => {
  ...
  } catch (error) { res.status(500).json({ error: 'URL fetch failed.' }); }
});
\`;
const start = code.indexOf(\"app.post('/api/video'\") !== -1 ? code.indexOf(\"app.post('/api/video'\") : code.indexOf('app.post(\"/api/video\"');
const end = code.indexOf('const SELF_URL');
if (start === -1 || end === -1) { console.log('Could not find markers. Start:', start, 'End:', end); process.exit(1); }
const newCode = code.slice(0, start) + newVideoRoutes + '\n' + code.slice(end);
