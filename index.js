...
function buildPrompt(userPrompt, mode) {
  ...
  if (mode === "model_capture") {
    return [
      ...
      "not named in the user brief, it does not exist in this image. Absent a",
      "brief-named setting, the viewer's blank backdrop is a studio backdrop, not a",
      ...
      "  (e) the environment: if the user brief below names a setting, site,",
      "      time of day, weather, or atmosphere, describe that setting believably",
      "      as the building's real surroundings, in positive language, per the",
      "      scenic-language rule below; otherwise write 'plain clear daytime sky,",
      "      flat neutral ground plane, soft even neutral daylight'.",
      "  Anything beyond these five parts is a defect.",
      "- Describe the output as a 'photorealistic architectural visualisation' — on",
      "  a seamless neutral background by default, or placed in the brief-named",
      "  setting when one is given; never as a generic 'photograph of a building',",
      "  which invites an unrequested invented site around it.",
...
function buildVideoPrompt(userPrompt, mode) {
  ...
  if (mode === "model_capture") {
    return clampVideoPrompt([
      ...
      "daylight and shadow, subtle motion of anything already visible in the",
      "source frame (existing planting, existing people), and — if named in the",
      "user brief below — a believable site, setting, or atmosphere around the",
      "building. Do not add any object, material, landscaping, or context that is",
      "neither already visible in the source frame nor named in the brief.",
  ...
  return clampVideoPrompt([
    ...
    "The only things permitted to change frame-to-frame: natural shift in",
    "daylight and shadow, subtle motion of anything already visible in the source",
    "frame, and — if named in the user brief below — a believable site, setting,",
    "or atmosphere around the building. Do not add any object, material,",
    "landscaping, or context that is neither already visible in the source frame",
    "nor named in the brief.",
